...
function IE({ value, onChange, placeholder="Tap to edit…", style={}, multiline=false }) {
  ...
  const props = {
    className:"inline-edit", value:val, placeholder,
...
// ─── SUPABASE HOOK ────────────────────────────────────────────────────────────
function useSupabase(table, initData, orderCol="id") {
  const [data, setData] = useState(initData);
  const [ready, setReady] = useState(false);

  useEffect(() => {
    sb.from(table).select("*").order(orderCol, {ascending:true})
      .then(({data:rows, error}) => {
        if (!error && rows && rows.length > 0) setData(rows);
        else if (!error && rows && rows.length === 0) {
          // seed initial data
          sb.from(table).insert(initData).then(() => setData(initData));
        }
        setReady(true);
      });
  }, [table]);
...
// HOME
// ═══════════════════════════════════════════════════════════════════════════════
function Home({ tasks, budget, budgetTotal, vendors, guests }) {
        ...
        <div style={{background:T.white,borderRadius:16,padding:16,marginBottom:12,border:`1px solid ${T.border}`}}>
          ...
          <div style={{display:"flex",gap:8,marginTop:10}}>
            <button onClick={()=>{
              ...
              setNewTask({text:"",cat:"Misc",priority:"medium"});setAdding(false);
...
// BUDGET
// ═══════════════════════════════════════════════════════════════════════════════
function Budget({ items, setItems, total, setTotal }) {
  const [adding, setAdding] = useState(false);
  ...
  const pct = total>0 ? Math.min(100,Math.round((totalSpent/total)*100)) : 0;

  const openEdit = item => { setEditingId(item.id); setEditForm({name:item.name,cat:item.cat,budget:String(item.budget),spent:String(item.spent)}); };
  const saveEdit = id => {
    setItems(b=>b.map(i=>i.id===id?{...i,name:editForm.name,cat:editForm.cat,budget:Number(editForm.budget)||0,spent:Number(editForm.spent)||0}:i));
    setEditingId(null);
  };
  const del = id => setItems(b=>b.filter(i=>i.id!==id));
  const addItem = () => {
    if(!nw.name.trim())return;
    setItems(b=>[...b,{id:Date.now(),name:nw.name,cat:nw.cat,budget:Number(nw.budget)||0,spent:Number(nw.spent)||0}]);
    setNw({name:"",cat:"Misc",budget:"",spent:""});setAdding(false);
  };
  ...
  return (
    <div style={{padding:"20px 16px 100px"}}>
      ...
      <div style={{background:`linear-gradient(135deg,${T.navy},${T.navyMid})`,borderRadius:18,padding:"18px 20px",marginBottom:16}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
          <div>
            ...
            <div style={{fontFamily:"'Clash Display',sans-serif",fontSize:22,fontWeight:700,color:T.white,marginTop:2}}>{fmt(total)}</div>
          </div>
        ...
        <div style={{display:"flex",justifyContent:"space-between"}}>
          ...
          <div style={{color:T.goldLight,fontSize:12,fontWeight:600}}>{fmt(total-totalSpent)} remaining</div>
        </div>
      </div>

      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:18}}>
        {[{label:"Allocated across items",val:fmt(totalAlloc),color:T.navyMid},{label:"Unallocated buffer",val:fmt(total-totalAlloc),color:T.success}].map(({label,val,color})=>(
        ...
        <div style={{background:T.white,borderRadius:16,padding:16,marginBottom:14,border:`2px solid ${T.gold}`}}>
          ...
          <div style={{display:"flex",gap:10,marginBottom:12}}>
            <div style={{flex:1}}>
              ...
              <input placeholder="500000" value={nw.budget} onChange={e=>setNw({...nw,budget:e.target.value})} type="number"
            ...
            <div style={{flex:1}}>
              ...
              <input placeholder="0" value={nw.spent} onChange={e=>setNw({...nw,spent:e.target.value})} type="number"
                style={{width:"100%",padding:"10px 12px",borderRadius:10,border:`1px solid ${T.border}`,fontSize:14,outline:"none"}}/>
            </div>
          </div>
          <div style={{display:"flex",gap:8}}>
      ...
      {items.map(item=>{
        const b=Number(item.budget)||0,s=Number(item.spent)||0;
            ...
            {!isEditing && (
              <div style={{padding:"14px 16px"}}>
                ...
                <div style={{display:"flex",gap:16,marginBottom:10}}>
                  <div><div style={{fontSize:11,color:T.muted}}>Budgeted</div><div style={{fontSize:15,fontWeight:700,color:T.charcoal}}>{fmt(b)}</div></div>
                  <div><div style={{fontSize:11,color:T.muted}}>Spent</div><div style={{fontSize:15,fontWeight:700,color:s>b?T.danger:T.success}}>{fmt(s)}</div></div>
                  <div><div style={{fontSize:11,color:T.muted}}>Left</div><div style={{fontSize:15,fontWeight:700,color:T.navyMid}}>{fmt(b-s)}</div></div>
                </div>
            ...
            {isEditing && (
              <div style={{padding:"14px 16px",background:"#FFFBF5",borderTop:`3px solid ${T.gold}`}}>
                ...
                <div style={{display:"flex",gap:10,marginBottom:12}}>
                  <div style={{flex:1}}>
                    <div style={{fontSize:12,color:T.muted,marginBottom:4}}>Budgeted (₹)</div>
                    <input value={editForm.budget} onChange={e=>setEditForm({...editForm,budget:e.target.value})} type="number"
                      style={{width:"100%",padding:"10px 12px",borderRadius:10,border:`1px solid ${T.border}`,fontSize:14,outline:"none"}}/>
                  </div>
                  <div style={{flex:1}}>
                    <div style={{fontSize:12,color:T.muted,marginBottom:4}}>Spent (₹)</div>
                    <input value={editForm.spent} onChange={e=>setEditForm({...editForm,spent:e.target.value})} type="number"
                      style={{width:"100%",padding:"10px 12px",borderRadius:10,border:`1px solid ${T.border}`,fontSize:14,outline:"none"}}/>
                  </div>
                </div>
                <div style={{display:"flex",gap:8}}>
        ...
        <div style={{background:T.white,borderRadius:16,padding:16,marginBottom:12,border:`1px solid ${T.border}`}}>
          ...
          <div style={{display:"flex",gap:8}}>
            <button onClick={()=>{
              ...
              setNv({cat:"Venue",name:"",contact:"",status:"searching",notes:""});setAdding(false);
  ...
  return (
    <div style={{padding:"20px 16px 100px"}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:14}}>
        {["Groom","Bride"].map(s=>(
          <button key={s} onClick={()=>setTab(s)} style={{flex:1,padding:"10px",borderRadius:12,cursor:"pointer",fontWeight:600,fontSize:14,background:tab===s?T.navy:T.white,color:tab===s?T.white:T.muted,border:`1px solid ${tab===s?T.navy:T.border}`}}>{s} Side ({guests.filter(g=>g.side===s).length})</button>
        ))}
      </div>
      {adding && (
        <div style={{background:T.white,borderRadius:16,padding:16,marginBottom:12,border:`1px solid ${T.border}`}}>
          ...
          <div style={{display:"flex",gap:8}}>
            <button onClick={()=>{
              ...
              setNg({name:"",from:"",side:"Groom",fn:"Wedding",hotel:"",rsvp:"pending",notes:""});setAdding(false);
      ...
      {filtered.length===0 && <div style={{textAlign:"center",color:T.muted,padding:"40px 0",fontSize:14}}>No guests yet on {tab} side.</div>}
      {filtered.map(g=>(
...
export default function WeddingOS() {
  ...
  const [tasks,   setTasks,   tasksReady]   = useSupabase("tasks",          INIT_TASKS);
  const [budgetItems, setBudgetItems, budgetReady] = useSupabase("budget_items", INIT_BUDGET_ITEMS);
  const [vendors, setVendors, vendorsReady] = useSupabase("vendors",        INIT_VENDORS);
  const [guests,  setGuests,  guestsReady]  = useSupabase("guests",         INIT_GUESTS);
  const [family,  setFamily,  familyReady]  = useSupabase("family_members", INIT_FAMILY);
  const [budgetTotal, setBudgetTotal] = useBudgetMeta();

  const allReady = tasksReady && budgetReady && vendorsReady && guestsReady && familyReady;

  if (!allReady) return (
    <>
      <style>{FONT_INJECT}</style>
      <div style={{maxWidth:480,margin:"0 auto",minHeight:"100vh",background:T.cream,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16}}>
        <div className="gold-shimmer" style={{fontFamily:"'Clash Display',sans-serif",fontSize:28,fontWeight:700}}>M & S</div>
        <div style={{color:T.muted,fontSize:13}}>Loading your wedding…</div>
      </div>
    </>
  );

  const SCREENS = {
    home:    <Home tasks={tasks} budget={budgetItems} budgetTotal={budgetTotal} vendors={vendors} guests={guests}/>,
    tasks:   <Tasks tasks={tasks} setTasks={setTasks}/>,
    budget:  <Budget items={budgetItems} setItems={setBudgetItems} total={budgetTotal} setTotal={setBudgetTotal}/>,
